...
import { LayoutDashboard, PieChart, LineChart, Wallet, Goal, Settings, BellRing, LogOut, Menu, Home, Bot } from "lucide-react";
...
const DashboardLayout = ({
  ...
  return <div className="min-h-screen w-full bg-gray-50">
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && <div className="fixed inset-0 z-50 bg-white lg:hidden">
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-auto p-4">
              <div className="space-y-6">
                ...
                {mainMenuItems.map(item => <Button key={item.label} variant={item.active ? "default" : "ghost"} className="w-full justify-start">
                ...
                {secondaryMenuItems.map(item => <Button key={item.label} variant="ghost" className="w-full justify-start">
                    <item.icon className="mr-2 h-5 w-5" />
                    {item.label}
                  </Button>)}
              </div>
      ...
      <SidebarProvider defaultOpen={true}>
        <div className="min-h-screen flex w-full">
            ...
            <SidebarContent>
              <SidebarGroup>
                ...
                <SidebarGroupContent>
                  <SidebarMenu>
                    <SidebarMenuItem>
                      <SidebarMenuButton isActive={false} tooltip="Página Inicial" onClick={() => navigate("/")}>
                        <Home />
                        <span>Página Inicial</span>
                      </SidebarMenuButton>
                    </SidebarMenuItem>

                    {mainMenuItems.map(item => <SidebarMenuItem key={item.label}>
              ...
              <SidebarGroup>
                ...
                <SidebarGroupContent>
                  <SidebarMenu>
                    {secondaryMenuItems.map(item => <SidebarMenuItem key={item.label}>
          ...
          <SidebarInset>
            <div className="hidden lg:flex justify-end items-center p-4 border-b rounded-sm">
              <Button variant="ghost" size="icon" onClick={handleNotificationClick}>
                <BellRing className="h-5 w-5" />
              </Button>
            </div>
